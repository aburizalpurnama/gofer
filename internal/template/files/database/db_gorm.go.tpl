...
import (
	...
	// Import pgx driver for native SQL if needed explicitly,
	// though database/sql usually handles this via the driver name string.
...
// NewGorm initializes a new GORM database connection with retry logic and configuration.
...
// NewNativeSQL initializes a new standard library sql.DB connection with retry logic.
func NewNativeSQL(cfg *config.Config) (*sql.DB, error) {
	dsn := getDSN(cfg)

	// sql.Open only prepares the DSN; it does not connect.
	// "pgx" is used as the driver name for PostgreSQL via github.com/jackc/pgx/v5/stdlib
